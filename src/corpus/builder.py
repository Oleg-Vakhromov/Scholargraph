...
class CorpusBuilder:
    ...
    def __init__(self, client) -> None:
        ...
        self._doi_attempted: set = set()
        self._venue_attempted: set = set()
        self._ref_attempted: set = set()
    ...
    def fetch_references(self, paper_ids=None, on_progress=None) -> pd.DataFrame:
        """
        ...
        Args:
            ...
            on_progress: Optional callable(current: int, total: int) called every
                         10 papers and on the final paper.
    ...
    def enrich_dois(self, crossref_client) -> tuple[int, int]:
        ...
        for idx, row in self.papers_df.iterrows():
            ...
            if paper_id in self._doi_attempted:
                continue
            self._doi_attempted.add(paper_id)
    ...
    def enrich_references(self, openalex_client) -> tuple[int, int]:
        ...
        for _, row in candidates.iterrows():
            ...
            if paper_id in self._ref_attempted:
                continue
            self._ref_attempted.add(paper_id)
            doi = row.get("doi")
    ...
    def enrich_venues(self, crossref_client) -> tuple[int, int]:
        ...
        for idx, row in self.papers_df.iterrows():
            ...
            paper_id = row.get("paper_id")
            if paper_id in self._venue_attempted:
                continue
            self._venue_attempted.add(paper_id)
            result = None
            doi = row.get("doi")
